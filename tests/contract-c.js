async function handle(state, action) {
    ...
    if (input.function === "claimB") {
        const result = await SmartWeave.contracts.write(state.b, {
            function: "claim",
            txID: input.txID,
            qty: 1,
        });
        if (result.type !== "ok") {
            throw new ContractError("Unable to run claimB.");
        }
        // If successful, push onto tokens[]
        state.tokens.push(state.b);
